...
/** -------------------- 测试 -------------------- */
test('自定义 Hook 模块边界检查器识别 JSX 模块中的函数、变量与嵌套实现', async () => {
  const diagnostics = await readCustomHookModuleDiagnostics([
    {
...
test('自定义 Hook 模块边界检查器接受组件内调用与非 JSX 实现', async () => {
  expect(await readCustomHookModuleDiagnostics([
    {
      filePath: 'component.tsx',
      source: `
        export function Component() {
