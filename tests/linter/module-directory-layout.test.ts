...
describe('模块目录布局守卫', () => {
  ...
  it('接受拍平、多并列模块、唯一 meta 模块骨架与 TanStack routes', () => {
    expect(readModuleDirectoryLayoutDiagnostics([
      ...
      { filePath: 'projects/server/src/app/define.ts', source: '' },
      { filePath: 'projects/server/src/app/orpc.ts', source: '' },
      { filePath: 'projects/server/src/app/router.ts', source: '' },
      { filePath: 'projects/server/src/app/server.ts', source: '' },
      { filePath: 'projects/server/src/infra/config/index.ts', source: '' },
      { filePath: 'projects/server/src/modules/meta/handlers.ts', source: '' },
